...
namespace CIE
{
    Vec3f spectrumToXYZ(const Spectrum &s, bool useCieCmf)
    {
        float X = 0.f, Y = 0.f, Z = 0.f;
        if (useCieCmf)
        {
            const CieTableEntry *t = cieTable();
        ...
        float scale = Spectrum::kStep / yBarIntegralFor(useCieCmf);
        return Vec3f(X * scale, Y * scale, Z * scale);
    }

    Vec3f spectrumToLinearSRGB(const Spectrum &s, bool useCieCmf)
    {
        return xyzToLinearSRGB(spectrumToXYZ(s, useCieCmf));
    }
}
